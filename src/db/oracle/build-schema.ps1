...
# Resyncs every identity column to max+1. Seed scripts insert explicit IDs into
# GENERATED BY DEFAULT AS IDENTITY columns without advancing the generator, so
# inserts/triggers would otherwise collide (ORA-00001). Must run BEFORE the
# trigger-firing dummy load (so audit triggers writing to EVENTS/_REV don't
# collide) AND at the end (so app inserts don't collide with dummy IDs).
$resyncBlock = @"
SET SERVEROUTPUT ON
BEGIN
...
$scripts = @(
    ...
    "phase_2.sql",
    "dental-release2.sql",
    "DENTAL-Check-all-coverage.sql",   # adds DENTAL_SERVICE_COVERAGE + CHECK_ALL_COVERAGE; rebuilds dental detail view/trigger
...
foreach ($s in $scripts) {
    ...
    # After objects/triggers exist but before the trigger-firing dummy load,
    # sync identities so audit triggers don't collide on EVENTS/_REV.
    if ($s -eq "3_db_objects_bkp.sql") {
        Write-Host "===== resyncing identities (pre-seed) =====" -ForegroundColor Green
        $resyncBlock | docker exec -i $Container sqlplus -S $Conn
    }
}

# Seed data inserts explicit IDs into identity columns without advancing the
# generator, so the next app/trigger insert collides (ORA-00001). Resync every
# identity to max+1. Then recompile every schema (objects are exported in
# dependency-agnostic order, so some are INVALID until their deps exist).
Write-Host "===== resyncing identities + recompiling schemas =====" -ForegroundColor Green
@"
SET SERVEROUTPUT ON
BEGIN
  FOR r IN (SELECT owner, table_name, column_name, generation_type
